...
#impute missing values with missForrest
# colSums(is.na.data.frame(allValues))
# gc()
# 
# 
# nc = detectCores()
# cl = makeCluster(min(12,nc))
# registerDoParallel(cl)
# imputedAllValues <- missForest(allValues[,-c(1,2)], verbose = TRUE, ntree = 30, parallelize ="forests")
# stopCluster(cl)
# 
# write.csv(imputedAllValues$ximp,"~/ForecastPH/Data/imputedAllValues.csv")
# colSums(is.na.data.frame(imputedAllValues$ximp))
# allValues<- imputedAllValues$ximp

# #load imputed values
# imputedAllValues <- read.csv("~/ForecastPH/Data/imputedAllValues.csv")
# #replace missing values
# allValues$current.capacity<-imputedAllValues$current.capacity
# allValues$total.capacityy<-imputedAllValues$total.capacity
# #check agian for missing values
colSums(is.na.data.frame(allValues))
allValues<-na.locf(allValues)
colSums(is.na.data.frame(allValues))
gc()
#add lagging (150 Lags)
allValues <-  tq_mutate(data = allValues,
...
write.csv(allValues,"~/ForecastPH/Data/finalData150.csv")
saveRDS(allValues,"~/ForecastPH/Data/finalData150.RDS")
gc()
