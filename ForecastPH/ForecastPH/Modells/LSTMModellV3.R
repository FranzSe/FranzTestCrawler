...
currentData<- filter(currentData,currentData$time>="2017-08-01")
...
saveRDS(currentData,"~/ForecastPH/Data/currentData.RDS")
...
saveRDS(col_means_train,"~/ForecastPH/Data/col_means_trainV3.RDS")
saveRDS(col_stddevs_train,"~/ForecastPH/Data/col_stddevs_trainV3.RDS")
...
build_model <- function() {
  ...
  model <- keras_model_sequential() %>%
    layer_dense(units = dim(train_df)[2], activation = "relu",
                input_shape = dim(train_df)[2]) %>%
    layer_dense(units = (dim(train_df)[2]/2), activation = "relu") %>%
...
save_model_hdf5(model, "~/ForecastPH/Modells/LSTM/LSTMModellV3.hdf", overwrite = TRUE,  include_optimizer = TRUE)
